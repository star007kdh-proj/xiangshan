...
class Ifu(implicit p: Parameters) extends IfuModule
  ...
  class IfuIO(implicit p: Parameters) extends IfuBundle {
    ...
    // Topdown analysis
    // Path B: base IfuPerfAnalysis uses the old-model topdownRedirect carried on FtqToIfuIO,
    // so no backendRedirectTopdown input (806ccd8c5 not cherry-picked).
  }
  ...
  perfAnalyzer.io.topdownIn.topdownRedirect := io.fromFtq.topdownRedirect
