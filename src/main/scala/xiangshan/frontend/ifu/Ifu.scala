...
class Ifu(implicit p: Parameters) extends IfuModule
  ...
  // let predecode check direct cfi targets itself instead of leaving it to the backend pc mem compare
  toFtq.predTargetQuery.zipWithIndex.foreach { case (query, i) =>
    query := s1_fetchBlock(i).ftqIdx
  }
  ...
  private val s2_predTarget   = RegEnable(fromFtq.predTarget, s1_fire)
  ...
  checkerIn.bits.blockPredTarget.zipWithIndex.foreach { case (predTarget, i) =>
    predTarget.valid := s2_fetchBlock(i).valid && s2_fetchBlock(i).takenCfiOffset.valid &&
      !s2_anyException && !s2_reqIsUncache
    predTarget.bits := s2_predTarget(i)
  }
